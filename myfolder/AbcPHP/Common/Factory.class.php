...
class Factory
{
	...
	public static function getDb($dbname = NULL, $dbClassName = 'MySql')
	{
		if (null == $dbname) {
			...
			$config = AbcPHPConfig::genDbConfig(C('DB_HOST'), C('DB_USER'), C('DB_PASSWORD'), C('DB_NAME'));
		} else {
			$dbId = $dbClassName . ':' . $dbname;
			$config = AbcPHPConfig::getDbConfig($dbname);
		}
	...
	public static function getDbByHost($host = '', $user = '', $pass = '', $dbname = '', $dbClassName = 'MySql')
	{
		...
		if (! isset(self::$DBS[$key]) || ! self::$DBS[$key]) {
			$config = AbcPHPConfig::genDbConfig($host, $user, $pass, $dbname);
			if (! $config) {
				return null;
			}
			$db = new $dbClassName($config['DB_HOST'], $config['DB_USER'], $config['DB_PWD'], $config['DB_NAME']);
	...
	public static function getCacher($type = '', $model = '')
	{
		...
		if (false == AbcPHPConfig::get('PUBLIC_SERVICE') && 'redis' == $type) {
			$type = 'file';
		}
		$cacheId = $type . $model;
		...
		switch ($type) {
			case 'file' :
				if (! class_exists("FileCache")) {
					include_once ABC_PHP_PATH . '/Cache/FileCache.class.php';
				}
				$c = new FileCache(C('RUN_SHELL'));
				$c->setModel($model);
				$c->setPath(AbcPHPConfig::getFileCacheDir());
				self::$CACHER[$cacheId] = $c;
				break;
			case 'remote' :
				if (! class_exists("FileCache")) {
					include_once ABC_PHP_PATH . '/Cache/RemoteCacher.class.php';
				}
			...
			default :
				if (! class_exists("RedisCache")) {
					include_once ABC_PHP_PATH . '/Cache/RedisCache.class.php';
				}
				$c = new RedisCache(AbcPHPConfig::get('REDIS_HOST'), AbcPHPConfig::get('REDIS_PROT'));
				self::$CACHER[$cacheId] = $c;
				break;
		}
		return self::$CACHER[$cacheId];
	...
	{
		if (! self::$SYS_LOG) {
			include_once ABC_PHP_PATH . '/Log/SystemLog.class.php';
			$obj = new SystemLog();
			$obj->setLogFormat(SystemLog::FORMAT_JSON);
			$obj->setLogpath(AbcPHPConfig::getSysLogDir());
			self::$SYS_LOG = $obj;
		}
		return self::$SYS_LOG;
	}
