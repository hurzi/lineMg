...
class LineApiRequest
{
    ...
    {
        $response = $this->_request($url, self::GET, $params);
     ...
     * @return mixed
     */
    public function post($url, $params = array(), $multi = false, $json = true)
    {
    	$timeout = 0;
		...
		if (true == $json) {
			$params = is_array($params) ? urldecode(json_encode($params)) : $params;
		}
        $response = $this->_request($url, self::POST, $params, $multi, $timeout);
        if (self::$format === 'json' && self::$decode_json) {
            $result = json_decode($response, true);
            if (! $result) {
            	$response = preg_replace("/[\\x00-\\x08\\x0b-\\x0c\\x0e-\\x1f]/", '', $response);
