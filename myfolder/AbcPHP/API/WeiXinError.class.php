...
final class WX_Error
{
     ...
     * @var INT
     */
    const INVALID_CARD_ID = 40073;
    /**
     * 核销卡券未到生效时间。
     * @var int
     */
    const INVALID_CARD_TIME = 40079;
    /**
     * 无效的卡券 序列号
     * @var INT
     */
    const INVALID_CARD_CODE = 40056;//invalid serial code
    
    const CARD_CODE_USED = 40099;//该 code 已被核销。
    /**
     * 没有菜单数据
     * @var int
     */
	...
	//API 授权后未确认
	const API_AUTH_NOT_CONFIRM = 48002;
        
	/**
	 * 卡券核销未到时间
	 * @var int
	 */
	const CARD_USE_INVALID_TIME = 40079;
	/*强授权相关*/
    const COMPONECT_MERCHANT_AUDITING = 60122;//审核中和通过的商户资料均不能重提
    const COMPONECT_ALREADY_CONFIRM = 61018;//已经 confirm 完成，不需重复 confirm
    const COMPONECT_NOT_SUBMIT_AUDIT = 61021;//不能 confirm。没有完成资料提交环节
    const COMPONECT_NOT_NEED_CONFIRM = 61019;//非强授权功能不需 confirm 确认
	public static function getMessage($code) {
		switch ($code) {
			...
			case self :: API_UNAUTHORIZED:
				...
				return ' api未授权';
			case self :: CARD_USE_INVALID_TIME:
				return ' 核销未到时间';
			case self::INVALID_CARD_ID:
				return ' 无效的卡券id';
			case self::INVALID_CARD_CODE:
				return ' 无效的卡券 序列号';
			case self::CARD_CODE_USED:
				return '该 code 已被核销';
			case self::INVALID_CARD_TIME:
				return '核销卡券未到生效时间';
			case self::API_AUTH_NOT_CONFIRM:
				return '授权应用后为未确认';
            case self::COMPONECT_MERCHANT_AUDITING:
                return '审核中和通过的商户资料均不能重提';
            case self::COMPONECT_ALREADY_CONFIRM:
            	return '已经 confirm 完成，不需重复 confirm';
            case self::COMPONECT_NOT_NEED_CONFIRM:
            	return '非强授权功能不需 confirm 确认';
            case self::COMPONECT_NOT_SUBMIT_AUDIT:
            	return '不能 confirm。没有完成资料提交环节';
		}
		return '未知错误';
	}
