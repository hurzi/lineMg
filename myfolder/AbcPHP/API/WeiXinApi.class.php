...
class WeiXinApi {
    ...
    /**
     * 上传图文素材
     * @param $news 图文数据 二维数组
     */
    public function uploadNews($news) {
        // 参数验证
        $articles = $this->_checkUploadNewsData($news);
        if (!$articles) {
            return false;
        }
     ...
     * 通过组群发消息 TODO:没测试
     */
    public function massMessageByGroup($massData) {
        if (@$massData['msgtype'] == 'text' && @$massData['text']['content']) {
            $massData['text']['content'] = $this->_textEncode($massData['text']['content']);
        }
        $params = $massData;
        $path = 'message/mass/sendall';
        $response = WeiXinApiRequest::post($this->_genUrl($path), $params);
        return call_user_func_array(array(
            $this,
            '_parse'
     ...
     * 通过用户列表发消息
     */
    public function massMessageByUserList($massData) {
        if (@$massData['msgtype'] == 'text' && @$massData['text']['content']) {
            $massData['text']['content'] = $this->_textEncode($massData['text']['content']);
        }
        $params = $massData;
    ...
    public function deleteMassMessage($msgId) {
        if (!$msgId) {
            $this->_setError(WX_Error::PARAM_ERROR);
            return false;
        }
        $params['msgid'] = $msgId;
        $path = 'message/mass/delete';
        $response = WeiXinApiRequest::post($this->_genUrl($path), $params);
        return call_user_func_array(array(
            $this,
            '_parse'
     ...
     * 上传素材图片获取url
     *
     * 对应API：{@link https://api.weixin.qq.com/cgi-bin/media/uploadimg}
     *
     * @author mxg
     * @param  string $image_name 图片路径
     * @return object WX_User
     */
    public function addMediaImg($image_name) {
        $params = array();
        $params['media'] = '@'.$image_name;
     ...
     */
    protected function _parseUploadPic($response) {
    ...
    protected function _sendMessage($message_body) {
        ...
        switch ($message_body->type) {
            ...
            case 'template' :
                ...
                if ($message_body->url) {
                    $params['url'] = $message_body->url;
                }
                if ($message_body->topcolor) {
                    $params['topcolor'] = $message_body->topcolor;
                }
    ...
    protected function _textEncode($text) {
        if (!$text) return null;
        return urlencode(str_replace(array('\\', '"'), array('\\\\', '\"'), $text));
    }
                ...
                if (!$message_body->media_id && !$message_body->attachment) {
                    ...
                    if (!$message_body->media_id) {
                        ...
                        if (!$attachment || preg_match($pregstr, $attachment)) {
                            ...
                            if ('voice' == $message_body->type && ('amr' != $ext || 'mp3' != $ext)) {
                                $is_error = true;
                            }
    ...
    protected function _log() {
        ...
        $params['info'] = array('appid' => $this->app_id,
     ...
     * @param array $news
     */
    private function _checkUploadNewsData($news) {
        $isError = false;
        $articles = array();
        if (!is_array($news)) {
            $isError = true;
            ...
            foreach ($news as $key => $value) {
                ...
                $value['author'] = $this->_textEncode($value['author']);
                $value['title'] = $this->_textEncode($value['title']);
                $value['content'] = $this->_textEncode($value['content']);
                $value['digest'] = $this->_textEncode($value['digest']);
                $value['content_source_url'] = urlencode($value['content_source_url']);
                $articles[$key] = $value;
            }
        }
     ...
     * @return array
     */
    protected function _parseUploadNews($response) {
        if (!isset($response['type']) || !isset($response['media_id'])) {
            $this->_setError(WX_Error::RESPONSE_FOMAT_ERROR);
            return false;
        }
     ...
     * @return mixed
     */
    private function _parseMassMessage($response) {
        if (!isset($response['errcode']) || !isset($response['errmsg']) || !isset($response['msg_id'])) {
            $this->_setError(WX_Error::RESPONSE_FOMAT_ERROR);
            return false;
        }
     ...
     * 根据订单id获取订单详情
     * TODO :
     * 对应API：{@link https://api.weixin.qq.com/merchant/order/getbyid:order_id}
     *
     * @author wangxiaohui Email:wangxiaohui@adsit.cn
     * @param string order_id
     * @return array
     */
    public function getbyOrderId($order_id) {

        $path = 'merchant/order/getbyid';
        $url = $this->_genUrl($path, false);
     ...
     * 根据订单创建时间及状态获取订单详情
     * TODO
     * 对应API：{@link https://api.weixin.qq.com/merchant/order/getbyfilter:order_id}
     *
     * @author wangxiaohui Email:wangxiaohui@adsit.cn
     * @param string status    订单状态
     * @param int str    订单创建开始时间
     * @param int end    订单创建结束时间
     * @return array
     */
    public function getbyFilter($status, $str, $end = 0) {
        ...
        $path = 'merchant/order/getbyfilter';
     ...
     * @return array
     */
    public function _parseJsApiTicket($response) {
     ...
     * 通过用户列表发消息
     */
    public function previewMessageByUserList($massData) {
        if (@$massData['msgtype'] == 'text' && @$massData['text']['content']) {
            $massData['text']['content'] = $this->_textEncode($massData['text']['content']);
        }
        $params = $massData;
        $path = 'message/mass/preview';
        $response = WeiXinApiRequest::post($this->_genUrl($path), $params);
        return call_user_func_array(array(
            $this,
            '_parse'
