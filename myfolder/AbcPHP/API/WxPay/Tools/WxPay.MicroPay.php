...
class MicroPay
{
	 ...
	 * @return 返回查询接口的结果
	 */
	public function pay($microPayInput,$payKey)
	{
		//①、提交被扫支付
		$result = WxPayApi::micropay($microPayInput,$payKey, 5);
		//如果返回成功
		...
		{
			$succResult = 0;
			$queryResult = $this->query($microPayInput,$payKey, $succResult);
			//如果需要等待1s后继续
			if($succResult == 2){
				sleep(2);
				continue;
	 ...
	 * @param int $succCode         查询订单结果
         * @param string $payKey       支付秘钥用于验证签名
	 * @return 0 订单不成功，1表示订单成功，2表示继续等待
	 */
	public function query($queryOrderInput,$payKey, &$succCode)
	{
//		$queryOrderInput = new WxPayOrderQuery();
//		$queryOrderInput->SetOut_trade_no($out_trade_no);
                $input = new WxPayMicroPay();
                $input->SetOut_trade_no($queryOrderInput->GetOut_trade_no());
                $input->SetAppid($queryOrderInput->GetAppid());//公众账号ID
                $input->SetMch_id($queryOrderInput->GetMch_id());//商户号
                $input->SetNonce_str($this->getNonceStr());//随机字符串
		
                $input->SetSign($payKey);//签名
		$result = WxPayApi::orderQuery($input,$payKey);
	 ...
	 * @param 调用深度 $depth
	 */
	public function cancel($out_trade_no,$payKey, $depth = 0)
	{
		if($depth > 10){
			return false;
		}
		...
		$result = WxPayApi::reverse($clostOrder,$payKey);
