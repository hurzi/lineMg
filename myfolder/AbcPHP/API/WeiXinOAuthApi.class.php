...
class WeiXinOAuthApi
{
	 ...
	 * @param string $cpAppId  组件APP ID
	 * @param string $cpAccessToken 组件授权TOKEN
	 * @return WX_OAuthToken the access token for oauth 2.0
	 */
	public function getCpAccessToken($code, $cpAppId, $cpAccessToken){
		$param = array(
				'appid' => $this->appId,
				'grant_type' => 'authorization_code',
				'component_appid' => $cpAppId,
				'component_access_token' => $cpAccessToken,
				'code'=> $code,
		);
		$url = $this->_getUrl('oauth2/component/access_token', $param);
		$response = WeiXinApiRequest::GET($url);
		return call_user_func_array(array($this, '_parse'),
				array(WeiXinApiRequest::$http_code, $response, '_parseToken'));
	}
	...
	protected function _log()
	{
		...
		Logger::error('WeiXinOAuthApi last erorr, url: '. WeiXinApiRequest::$url, $params);
	}
}