...
_extend(KWidget, {
	...
	pos : function(x, y, updateProp) {
		...
		if (y !== null) {
			//周宣修改弹出层高度 2013-6-17
			y = y < 0 ? 0 : _addUnit($(parent.document).scrollTop()+50);//parent.document.documentElement.scrollTop+50);//_addUnit(y);
			self.div.css('top', y);
			if (updateProp) {
				self.y = y;
			}
...
KEditor.prototype = {
	...
	fullscreen : function(bool) {
		...
		if(this.fullscreenMode === false){
			this.pScrollTop = top.window.$('html').scrollTop();
			top.window.$('html').scrollTop(0);
		}else{
			if(this.pScrollTop !== undefined ){
				top.window.$('html').scrollTop(this.pScrollTop);
			}
		}
