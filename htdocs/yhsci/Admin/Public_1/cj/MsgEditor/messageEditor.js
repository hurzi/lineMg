$(function() {
	$('.Yfs').live('click', function() {
		var height = $('.Yfs').offset().top;
		maptss('发送预览', 'Message/session.html', height - 200);
	}); // iframe弹出层

	// 编辑按钮
	$('.Cledit').live('mouseenter', function() {
		$(this).find('.abs').css({
			'left' : '0px'
		});
	});
	$('.Cledit').live("mouseleave", function() {
		$(this).find('.abs').css({
			'left' : '100%'
		});
	});
	
	function maptss(title, url, topfix, conw, conh) {
		if (!conw) {
			conw = 400;
		};
		if (!conh) {
		...
		new jsbox({
			content : '<div style="margin-left:30px;margin-top:24px;height:50px"><span class="twp" style="font-size:13px;">请输入微信昵称:</span><input class="msg-input" style="display:inline;width:150px"type="text" id="wx_number"/></div>',
			onlyid : "maptss",
			title : title,
			footer : true,
			conw : conw,
			Ok_button : 'send_preview_message()',
			// conh:conh,
			FixedTop : topfix,
			iframe : false,
			range : true,
...
Abc.MessageEditor = function(type, data, param) {
	...
	this.upload_url = param.upload_url
			|| './UploadImg/Upload.php?callback=Abc.MessageEditor.uploadImg';
...
Abc.MessageEditor.prototype.getFrameView = function() {
	var tpl = '<div class="Mat_con" style="word-wrap: break-word;">'
			+ '<div class="mat_l">' + '<div class="TW_box">'
			+ '<div class="appTwb2">' + '</div>' + '</div>' + '</div>'
			+ '<div class="mat_r">' + '</div>' + '</div>';
	return tpl;
};
...
Abc.MessageEditor.prototype.initUeditor = function() {
	...
	var editorOption = {
		...
		this.Ueditor = new UE.ui.Editor(editorOption);
		this.Ueditor.render(Abc.MessageEditor.EDITOR_ID);
		//UE.getEditor(Abc.MessageEditor.EDITOR_ID,editorOption);
...
Abc.MessageEditor.prototype.getFormTpl = function() {
	var tpl = '<div class="rel msg-editer-wrapper">'
			...
			+ '<span id="upload-tip" class="upload-tip r">效果建议尺寸：大图360*200，小图200*200</span>'
			+ '封面'
			+ '</label>'
			+ '<div class="cover-area" style="width:309px;">'
			...
			+ '<input type="checkbox" class="msg-input" style="width:auto;display:inline-block;margin-left:5px;" id="show_cover_pic" value=""></label>'
			+ '<label for="" class="block">摘要&nbsp;<span style="color:#B8B8B8;">(摘要不能超过'
			+ (this.desc_max / 2)
			+ '个字)</span></label>'
			...
			+ '<div style="height:360px;"><script type="text/plain" id="'+Abc.MessageEditor.EDITOR_ID+'"></script></div>'
...
Abc.MessageEditor.prototype.lislenTpl = function() {
	...
	$("#delImg").die().bind("click", function() {
		_this.delImg();
	});
};
// 验证输入字数（中英）
...
Abc.MessageEditor.prototype.lislenPreview = function() {
	...
	if (this.type_ == 2) {
		// 添加新news
		$('.sub-add-btn').die().live('click', function() {
			if (_this.preview.news_count >= 10) {
				loadMack({
					off : 'on',
					Limg : 0,
					text : '无法添加，多条图文时，数据最多为10条。',
					set : 1000
				});
				return;
			}
			...
			$('.mat_r .msg-editer-wrapper').css({
				'marginTop' : top_value - 68
			}).show();
			parentSH();
		});

		// 删除news
		$('.iconDel').die().live('click', function() {
			// 保存正在操作的表单的数据
			_this.saveEditorData(_this._index);
		...
		// 编辑头条图文
		$('.appTwb1 .iconEdit').die().live('click', function() {
			_this.saveEditorData(_this._index);
			_this._index = 0;
			_this.showDataToEditor(_this._index);
			$('.mat_r .msg-editer-wrapper').css({
		...
		// 编辑后面图文
		$('.appTwb2 .iconEdit').die().live('click', function() {
			var Pobj = $(this).parents('.tw_li');
			_this.saveEditorData(_this._index);
...
Abc.MessageEditor.checkData = function(data) {
	for ( var i = 0; i < data.length; i++) {
		...
		/*
		 * if(data[i]['describe'] == ''){
		 * loadMack({off:'on',Limg:0,text:'第'+(i+1)+'条图文的描述不能为空。',set:1000});
		 * return false; }
		 */
		// if(data[i]['news_url'] == ''){
		// loadMack({off:'on',Limg:0,text:'第'+(i+1)+'条图文的链接不能为空。',set:1000});
		// return false;
		// }
