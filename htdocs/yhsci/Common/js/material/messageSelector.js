...
MessageSelector.prototype._listen = function() {
	// 确定提交按钮事件处理
	$(this._getId(MessageSelector.DEFINE.SUBMIT_BTN, '#')).off().click(MessageSelector.bind(this.submitMaterial, this));
	//pull_down按钮事件处理
	if(this._param.mode == 'pull_down'){
		$(this._getId(MessageSelector.DEFINE.MENU, '#')).off().click(function(){
			var $obj = $(this);
			$obj.addClass('jda');
	...
	if (this._param.mode == 'button') {
		obj = $(this._getId(MessageSelector.DEFINE.MENU, '#') + ' >li');
...
MessageSelector.prototype._selectedCallback = function(materialId, html, msgType) {
	...
	$(this._getId(msgType+MessageSelector.DEFINE.MATERIAL_CONTENT, '#')).attr(MessageSelector.DEFINE.MSG_TYPE, msgType).html(html || '');
...
MessageSelector.check = function(selectData) {
	...
	if ('text' == msgType) {
		if (isEmpty(textContent)) {
			Common.alert('文字消息不能为空');
			return false;
		}
	} else {
		if (isEmpty(materialId) || materialId == 0) {
			Common.alert('请选择消息素材');
			return false;
		}
	}
	return true;
...
// 切换消息类型标签
MessageSelector.prototype._switchTab = function(msgType) {
	// TODO mod
	if (this._param.mode == 'button') {
		...
		$(this._getId(MessageSelector.DEFINE.MENU, '#')).find('li')
				.removeClass('active').parent().find(
						this._getId(MessageSelector.DEFINE.MENU_ID, '#' + msgType + '_')).addClass('active');
	}

	//切换内容tab
	$(this._getId(MessageSelector.DEFINE.BODY, '#')).find('.tab-pane')
				.removeClass('active').parent().find(
						this._getId(msgType+MessageSelector.DEFINE.MATERIAL_CONTENT_BOX,"#")).addClass('active');
	
	this._selectedData[MessageSelector.DATA.TYPE] = msgType;
	if (this._param.oauth_set && msgType == 'news') {
		$(this._getId(msgType+MessageSelector.DEFINE.OAUTH_SET_CON, "#")).show();
	}
};
...
MessageSelector.prototype._createTabFrame = function() {
	var uiStr = '<div class="tabbable">';
	// tab菜单栏
	uiStr += '<ul class="nav nav-tabs nav-tabs-lg" id="' + this._getId(MessageSelector.DEFINE.MENU) + '"></ul>';
	uiStr += '<div class="tab-content" id="' + this._getId(MessageSelector.DEFINE.BODY) + '">';
...
MessageSelector.prototype._createTabMenu = function() {
	...
	for ( var i = 0, len = this._param.msg_types.length; i < len; i++) {
		var boxId = this._getId(this._param.msg_types[i]+MessageSelector.DEFINE.MATERIAL_CONTENT);
		var css = this._param.msg_types[i] == msgType ? 'class="active"' : '';
		var id = this._getId(MessageSelector.DEFINE.MENU_ID, this._param.msg_types[i] + '_');

		menuStr += '<li ' + css+' id='+id+'>';
		menuStr += '<a  data-toggle="tab" href="#'+boxId+'">'+this._param.msg_type_names[this._param.msg_types[i]]+'</a>';
		menuStr += '</li>';		
	}
	return menuStr;
};
...
MessageSelector.prototype._createBody = function() {
	...
	for ( var i = 0, len = this._param.msg_types.length; i < len; i++) {
		...
		var boxId = this._getId(this._param.msg_types[i]+MessageSelector.DEFINE.MATERIAL_CONTENT_BOX);
		var textContentId = this._getId(MessageSelector.DEFINE.TEXT_CONTENT);		
		var malContentId = this._getId(this._param.msg_types[i]+MessageSelector.DEFINE.MATERIAL_CONTENT);		
		var hidecss = this._param.msg_types[i] == msgType ? 'tab-pane active' : 'tab-pane';
		var oauthSetId = this._getId(this._param.msg_types[i]+MessageSelector.DEFINE.IS_OAUTH);
		var oauthSetCon = this._getId(this._param.msg_types[i]+MessageSelector.DEFINE.OAUTH_SET_CON);
		bodyStr += '<div id="' + boxId + '" class="' + hidecss + '">';
		switch(this._param.msg_types[i]){
		case 'text':
			bodyStr += '<div id="'+this._getId(MessageSelector.DEFINE.WX_FACE_ID)+'" class="face_btn"></div>';
			bodyStr += '<textarea id="' + textContentId + '" rows="5" class="form-control">' + (this._data ? (this._data[MessageSelector.DATA.CONTENT] || '') : '') + '</textarea>';
			bodyStr += '</div>';
			break;
		default:
			bodyStr += '<div id="' + malContentId + '">' + materialContent + '</div>';
...
MessageSelector.prototype._createInitMessage = function() {
	...
	case "music":
		...
		msgHtml += '<div class="twp3" style="height: 100px;width: 100px;float:left;overflow:hidden;">';
...
MessageSelector.URLS = {
	GET_NEWS_URL : '../Manage/index.php?a=WxMaterialNews&m=showMaterial', //获取图文url
	GET_MUSIC_URL : '../Manage/index.php?a=WxMaterialMusic&m=showMaterial', //获取音乐url
	GET_IMAGE_URL : '../Manage/index.php?a=WxMaterialImage&m=showMaterial', //获取图片url
	GET_VOICE_URL : '../Manage/index.php?a=WxMaterialVoice&m=showMaterial', //获取语音url
	GET_VIDEO_URL : '../Manage/index.php?a=WxMaterialVideo&m=showMaterial' //获取视频url
};